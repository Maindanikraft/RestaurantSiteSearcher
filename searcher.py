...
sorted by opportunity score (reviews x rating weight -- missing info penalty).
"""
...
class Business:
    def __init__(self, name, rating, user_ratings_total, phone, address, place_id, types, maps_url=""):
        self.name = name
        self.rating = rating
...
def geocode_location(api_key, location_str):
...
def search_businesses(api_key, lat, lng, radius_m, keyword, max_results):
    results = []
    params = {
        "location": f"{lat},{lng}",
        "radius": radius_m,
...
def write_csv(businesses, output_path):
    if not businesses:
